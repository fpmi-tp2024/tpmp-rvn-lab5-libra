...
3. **CarStorer tests** - проверяют корректность работы класса `CarStorer`, который предоставляет методы для работы с автомобилями в базе данных.
4. **OrderStorer tests** - проверяют корректность работы класса `OrderStorer`, который предоставляет методы для работы с заказами в базе данных.
5. **DriverStorer tests** - проверяют корректность работы класса `DriverStorer`, который предоставляет методы для работы с водителями в базе данных.
...
В этой группе тестов создается набор тестовых данных для базы данных приложения. В частности, создаются водители, автомобили и заказы. Для каждого водителя создается уникальный логин и пароль, а также указываются имя, категория, дата рождения и адрес. Для каждого автомобиля указываются номер, марка, модель, пробег и грузоподъемность. Для каждого заказа указываются дата, номер автомобиля, водитель, пробег, масса груза и стоимость.
